...
> Must confirm before treating as spec: **is this hospital contact the same "พี่" who owns the original project, or a new front-line user?** (Note — confirmed by the instructor since Sep 9, 2026: the W5 Gate doesn't require a literal interview headcount — see `.docs/01-requirements/backlog.md` for the clarified framing — but this identity question still matters for knowing whose perspective this interview represents.)
...
- **Dashboard for viewing the data kept for analysis: still unconfirmed.** The team itself isn't sure it's needed — consistent with the team already parking AdminDashboard as a stretch goal, not core MVP (see "Feature scope" below). Storing data in the DB with query/export support is enough for MVP for now
...
3. **Part 3.0 (symptom triggers):** vomiting/diarrhea present, or temporary malnutrition present → advise Sick Day Rules → system prompts the screener to give that advice; **a chatbot is named here** ("มี chat bot ที่สามารถให้คำแนะนำหรือเป็นที่ปรึกษาเมื่อผู้ป่วยไม่สบาย") — this is the first real pain source for the chatbot idea (see PBI-S3 in the backlog). **Team-confirmed: patient-facing** — patients chat with it directly (see "Confirmed workflow model" above)
...
- ~~Is the Part 3.0 chatbot staff-facing or patient-facing?~~ — **answered (team decision): patient-facing.** Patients chat with it directly (e.g. about MALA / their medication). Note this doesn't resolve *what UX/UI it should look like* — still needs designing, and it stays out of MVP (see `.docs/04-build/scope-lock.md`)
